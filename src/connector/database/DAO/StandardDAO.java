...
import connector.database.model.BaseModel;
...
public abstract class StandardDAO {
	...
	public boolean deleteModel(Object objectModel)
	{
		...
		try {
			...
			List<BaseModel> sInfoList = session.createQuery("from "+ clazzName + " where id = :value").setInteger("value", baseModel.getId()).list();
			
			if (sInfoList != null)
			{
				for (BaseModel f : sInfoList)
				{
					session.delete(f);
				}
			}
